...
class TestLogic(unittest.TestCase):
    ...
    def test_handle_message_routine_status(self):
        reset_routines()
        handle_message(None, None, None, b'{"type": "routine_status", "routine": "test", "status": "test_status"}')
        self.assertEqual(get_state("test"), {"status": "test_status", "tasks": {}})
        handle_message(None, None, None, b'{"type": "routine_status", "routine": "test", "status": "test_status2"}')
        self.assertEqual(get_state("test"), {"status": "test_status2", "tasks": {}})
    ...
    def test_flow(self):
        ...
        self.assertEqual(get_state("test"), {"status": "test_status", "tasks": {}})
        handle_message(None, None, None, b'{"type": "task_status", "routine": "test", "task_id": 12, "status": "test_status"}')
        self.assertEqual(get_state("test"), {"status": "test_status", "tasks": {12: "test_status"}})

    @patch('logging.error')
    def test_handle_message_assert_exception(self, mock_log_error):
        reset_routines()
        with self.assertRaises(AssertionError):
            handle_message(None, None, None, b'{"type": "routine_status", "routine": null, "status": "test_status"}')
